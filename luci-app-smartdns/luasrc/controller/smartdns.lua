...
function index()
	...
	local page
	page = entry({"admin", "dns", "smartdns"}, cbi("smartdns/smartdns"), _("SmartDNS"), 60)
	page.dependent = true
	page = entry({"admin", "dns", "smartdns", "status"}, call("act_status"))
	page.leaf = true
	page = entry({"admin", "dns", "smartdns", "upstream"}, cbi("smartdns/upstream"), nil)
	page.leaf = true
end
